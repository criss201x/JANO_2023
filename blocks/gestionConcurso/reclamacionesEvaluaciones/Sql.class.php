	...
	function getCadenaSql($tipo, $variable = "") {
		...
		switch ($tipo) {
			...
			case 'buscarSoporte' :
				...
				case "ConsultaReclamacionFinal" :
					$cadenaSql="SELECT er.consecutivo_calendario, er.id_inscrito, aa.consecutivo_actividad ";
					$cadenaSql.="FROM concurso.evaluacion_reclamacion er ";
					$cadenaSql.="inner join concurso.concurso_calendario cc on er.consecutivo_calendario = cc.consecutivo_calendario ";
					$cadenaSql.="inner join concurso.actividad_academica aa on cc.consecutivo_actividad = aa.consecutivo_actividad ";
					$cadenaSql.=" WHERE er.id=".$variable ['reclamacion'];
					//echo $cadenaSql;
					break;
				...
				case "consultarDetalleReclamacion2" :
					...
					$cadenaSql .= " FROM concurso.evaluacion_reclamacion reclamacion, concurso.evaluacion_parcial evaluacion, concurso.".$variable['tipoEvaluacion']." ce, concurso.criterio_evaluacion criterio, concurso.evaluacion_grupo grupo, jano_usuario us";
					$cadenaSql .= " WHERE";
					$cadenaSql .= " reclamacion.id=evaluacion.id_reclamacion";
					$cadenaSql .= " and evaluacion.id_evaluar=ce.consecutivo_evaluar";
					$cadenaSql .= " and ce.consecutivo_criterio=criterio.consecutivo_criterio";
				...
				case "consultarReclamacionConcurso" :
					$cadenaSql = " SELECT distinct recl.id, recl.consecutivo_calendario, recl.observacion reclamo , ac.consecutivo_actividad, ";
					$cadenaSql .= " recl.id_inscrito, rsta.id id_rsta, (CASE WHEN rsta.respuesta='SI' THEN 'Aplica' ELSE 'No aplica' END) resultado, ";
					$cadenaSql .= " rsta.observacion, rsta.fecha_registro, rsta.id_evaluar_respuesta, rsta.id_evaluador ";
					$cadenaSql .= " FROM concurso.evaluacion_reclamacion recl ";
					$cadenaSql .= " left JOIN concurso.respuesta_reclamacion rsta ON recl.id=rsta.id_reclamacion AND recl.estado=rsta.estado ";
					$cadenaSql .= " inner join concurso.concurso_calendario cc on recl.consecutivo_calendario = cc.consecutivo_calendario ";
					$cadenaSql .= " inner join concurso.actividad_calendario ac on cc.consecutivo_actividad = ac.consecutivo_actividad ";
					$cadenaSql .= " WHERE rsta.estado='A' ";
					$cadenaSql .= " AND recl.id=" . $variable ['reclamacion'];
					//$cadenaSql .= " AND evaluacion.estado='A'";
					//echo $cadenaSql;
					break;
			...
			case "consultaConcurso" :
				...
				$cadenaSql .= " AND reclamacion.consecutivo_calendario=calendar.consecutivo_calendario";
                                $cadenaSql .= " AND  reclamacion.estado='A' ";
			...
			case "consultaReclamaciones" :
				...
				break;
                            ...
                            case "consultarReclamaciones":
                                    ...
                                    if(isset($variable ['jurado'])){
                                            $cadenaSql .= " INNER JOIN concurso.jurado_inscrito jur ON jur.id_inscrito=ci.consecutivo_inscrito AND jur.id_usuario='".$variable['jurado']."'";
                                        }
                                    
                                    $cadenaSql.=" WHERE ";
                                    $cadenaSql.=" cp.consecutivo_concurso=".$variable['consecutivo_concurso'];
                                    $cadenaSql.=" AND act.nombre!='Evaluar requisitos'";
                                    $cadenaSql.=" AND  er.estado='A' ";
                                    if(isset($variable ['actividad'])){
